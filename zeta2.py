...
Ri = 0. + 0.j #N**2./omg**2. # Richardson number
...
for i in range(0,Ngrid):
  for j in range(0,Ngrid):
    ...
    params = {'nu': nu, 'kap': kap, 'omg': omg, 'L':L, 'T': T, 'Td': Td, 'Nt':Nt, 'U': U, 
          'N':N, 'tht':tht[i], 'Re':Re, 'Ri':Ri,
          'H': H, 'Hd': Hd, 'Nz':Nz, 'wall':wall_flag,
          'dS':dS, 'ReS':ReS, 'grid':grid_flag, 'f': f, 'Pr':Pr,
          'z':z, 'dz':dz, 'Ro':Ro, 'k0':k0, 'l0':l0, 'CFL':CFL}

    Phi0 = np.eye(int(2*Nz),int(2*Nz),0,dtype=complex) # initial condition (prinicipal fundamental solution matrix)
