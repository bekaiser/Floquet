...
for n in range(0, Ng): 
    ...
    # test signals:
    b = np.cos(2.*np.pi/H*z)
    b_z = -2.*np.pi/H*np.sin(2.*np.pi/H*z)
    b_zz = -np.cos(2.*np.pi/H*z)*(2.*np.pi/H)**2.
    ...
    if n == Ng-1:
        plotname = figure_path + 'psi_solutions.png'
        ...
        #plt.plot(abs(uzz-uzzD)/abs(uzz)*100.,z/H,'b')
        plt.xlabel(r"$\partial_{zz}\zeta=\partial_{zz}(\partial_{zz}\psi)$", fontsize=16); plt.grid()
        #plt.ylabel(r"$z/H$",fontsize=16); plt.grid()
        #plt.title(r"uniform grid N = %i" %(Nz),fontsize=13)
        #plt.legend(loc=2,fontsize=13); 
        ...
        plotname = figure_path + 'b_solutions.png'
        fig = plt.figure(figsize=(12,4.5)); 
        plt.subplot(1,3,1)
        plt.plot(b, z/H,color='darkturquoise',linewidth=2) #, label=r"u")
        #plt.plot(abs(uzz-uzzD)/abs(uzz)*100.,z/H,'b')
        ...
        #plt.axis([0.,20./H,0.,1./(2.*H)])
        plt.subplot(1,3,2)
        plt.plot(b_z, z/H,color='darkturquoise',linewidth=2) #, label=r"analytical")
        #plt.plot(abs(uzz-uzzD)/abs(uzz)*100.,z/H,'b')
        plt.xlabel(r"$\partial_{z}b$", fontsize=16); plt.grid()
        #plt.ylabel(r"$z/H$",fontsize=16)
        #plt.title(r"uniform grid N = %i" %(Nz),fontsize=13)
        plt.subplot(1,3,3)
        plt.plot(b_zz, z/H,color='darkturquoise',linewidth=2) #, label=r"analytical")
        #plt.plot(abs(uzz-uzzD)/abs(uzz)*100.,z/H,'b')
        plt.xlabel(r"$\partial_{zz}b$", fontsize=16); plt.grid()
        #plt.ylabel(r"$z/H$",fontsize=16); plt.grid()
        #plt.title(r"uniform grid N = %i" %(Nz),fontsize=13)
        plt.subplots_adjust(top=0.97, bottom=0.15, left=0.085, right=0.98, hspace=0.08, wspace=0.2)
        plt.savefig(plotname,format="png")
        plt.close(fig);
    ...
    #print(np.mean(abs(psi_zz)))
    psi_zzE = abs(psi_zz-psi_zzC) / np.mean(abs(psi_zz))
    ...
    zeta_zzE = abs(psi_zzzz-zeta_zzC) / np.amax(abs(psi_zzzz))
    L_zeta_zz[n] = np.amax(zeta_zzE)
    ...
    psiE = abs(psi-psiC) / np.amax(abs(psi))
    #print(np.mean(abs(psi)))
    ...
    # now do exactly as in the solutions:
    wall_BC_flag = ' '
    grid_params_b = {'H':H, 'Hd':Hd,'z':z,'dz':dz,'Nz':Nz, 'wall_BC_flag':wall_BC_flag} 
    ...
    b_zC = np.dot( dz_b , b )
    b_zE = abs(b_z-b_zC) / np.amax(abs(b_z))
    L_b_z[n] = np.amax(b_zE)

    b_zzC = np.dot( dzz_b , b )
    b_zzE = abs(b_zz-b_zzC) / np.amax(abs(b_zz))
    L_b_zz[n] = np.amax(b_zzE)



    plotname = figure_path + '/computed_solutions/b_dz_Nz%i.png' %(int(Nz))
    fig = plt.figure(figsize=(16,8)); 
    plt.subplot(1,2,1)
    plt.plot(b_z, z/H, color='red', linewidth=2, label=r"analytical")
    plt.plot(b_zC, z/H, linestyle='dashed',color='olive',linewidth=2, label=r"computed")
    #plt.plot(uzzDb, z/H, '--g', label=r"dirchlet")
    #plt.plot(abs(uzz-uzzD)/abs(uzz)*100.,z/H,'b')
    plt.xlabel(r"$\partial_{z}b$", fontsize=13) # $\mathrm{percent}$ $\mathrm{error}$", fontsize=13)
    plt.ylabel(r"$z/H$",fontsize=13); plt.grid()
    plt.title(r"uniform grid N = %i" %(Nz),fontsize=13)
    plt.legend(loc=1,fontsize=13); 
    #plt.ylim([-0.005,5.])
    ...
    plotname = figure_path + '/computed_solutions/b_dzz_Nz%i.png' %(int(Nz))
    fig = plt.figure(figsize=(16,8)); 
    plt.subplot(1,2,1)
    plt.plot(b_zz, z/H, color='red', linewidth=2, label=r"analytical")
    plt.plot(b_zzC, z/H, linestyle='dashed',color='olive',linewidth=2, label=r"computed")
    #plt.plot(uzzDb, z/H, '--g', label=r"dirchlet")
    #plt.plot(abs(uzz-uzzD)/abs(uzz)*100.,z/H,'b')
    plt.xlabel(r"$\partial_{zz}b$", fontsize=13) # $\mathrm{percent}$ $\mathrm{error}$", fontsize=13)
    plt.ylabel(r"$z/H$",fontsize=13); plt.grid()
    plt.title(r"uniform grid N = %i" %(Nz),fontsize=13)
    plt.legend(loc=1,fontsize=13); 
    #plt.ylim([-0.005,5.])
...
"""
plotname = figure_path + 'bz_convergence.png'
fig = plt.figure(figsize=(8,8))
plt.subplot(1,1,1)
plt.loglog(Nr,L_b_z,color='olive',linewidth=2.,label=r"$\partial_{z}b$")
#plt.loglog(Nr,Lb,'b',label=r"cosine grid")
plt.loglog(Nr,(L_b_z[0]*0.5/Nr[0]**(-2.))*Nr**(-2.),'k',label=r"$O(N^{-2})$")
...
plotname = figure_path + 'bzz_convergence.png'
fig = plt.figure(figsize=(8,8))
plt.subplot(1,1,1)
plt.loglog(Nr,L_b_zz,color='olive',linewidth=2.,label=r"$\partial_{zz}b$")
#plt.loglog(Nr,Lb,'b',label=r"cosine grid")
plt.loglog(Nr,(L_b_zz[0]*0.5/Nr[0]**(-2.))*Nr**(-2.),'k',label=r"$O(N^{-2})$")
plt.xlabel(r"$N$ grid points",fontsize=13)
plt.ylabel(r"L$_\infty$ error",fontsize=13)
#plt.title(r"$\partial_{zz}\psi$ error",fontsize=13) #: Dirchlet top & open bottom BCs",fontsize=13)
plt.grid(); plt.legend(loc=1,fontsize=13)
...
plotname = figure_path + 'psi_grid_convergence.png'
...
plot_xlabel = r"$N$ $\mathrm{grid}$ $\mathrm{points}$, $\mathrm{uniform}$ $\mathrm{grid}$"
...
plt.grid(); plt.legend(loc=3,fontsize=16)
plt.subplot(1,3,2)
plt.loglog(Nr,L_zeta_zz,color='goldenrod',linewidth=2.,label=r"$\partial_{zz}\zeta$")
#plt.loglog(Nr,Lb,'b',label=r"cosine grid")
plt.loglog(Nr,(L_zeta_zz[0]*0.005/Nr[0]**(-2.))*Nr**(-2.),'k',label=r"$O(N^{-2})$")
#plot_xlabel = r"$N$ $\mathrm{grid}$ $\mathrm{points}$, " + grid_flag + " $\mathrm{grid}$"
plot_xlabel = r"$N$ $\mathrm{grid}$ $\mathrm{points}$, $\mathrm{uniform}$ $\mathrm{grid}$"
plt.xlabel(plot_xlabel,fontsize=16)
plt.yticks([1e-6,1e-4,1e-2,1e-0,1e2])
#plt.ylabel(r"L$_\infty$ $\mathrm{error}$",fontsize=16)
...
plotname = figure_path + 'b_grid_convergence.png'
fig = plt.figure(figsize=(10,5)); 
plt.subplot(1,2,1)
plt.loglog(Nr,L_b_z,color='darkturquoise',linewidth=2.,label=r"$\partial_{z}b$")
#plt.loglog(Nr,Lb,'b',label=r"cosine grid")
plt.loglog(Nr,(L_b_z[0]*0.5/Nr[0]**(-2.))*Nr**(-2.),'k',label=r"$O(N^{-2})$")
#plot_xlabel = r"$N$ $\mathrm{grid}$ $\mathrm{points}$, " + grid_flag + " $\mathrm{grid}$"
plot_xlabel = r"$N$ $\mathrm{grid}$ $\mathrm{points}$, $\mathrm{uniform}$ $\mathrm{grid}$"
...
plt.ylabel(r"L$_\infty$ $\mathrm{error}$",fontsize=16)
plot_title = r"$\mathrm{Buoyancy}$ $1^{st}$ $\mathrm{derivative}$ $\mathrm{error}$"
plt.title(plot_title,fontsize=16)
plt.grid(); plt.legend(loc=3,fontsize=16)
plt.subplot(1,2,2)
plt.loglog(Nr,L_b_zz,color='darkturquoise',linewidth=2.,label=r"$\partial_{zz}b$")
#plt.loglog(Nr,Lb,'b',label=r"cosine grid")
plt.loglog(Nr,(L_b_zz[0]*0.5/Nr[0]**(-2.))*Nr**(-2.),'k',label=r"$O(N^{-2})$")
#plot_xlabel = r"$N$ $\mathrm{grid}$ $\mathrm{points}$, " + grid_flag + " $\mathrm{grid}$"
plot_xlabel = r"$N$ $\mathrm{grid}$ $\mathrm{points}$, $\mathrm{uniform}$ $\mathrm{grid}$"
plt.xlabel(plot_xlabel,fontsize=16)
#plt.ylabel(r"L$_\infty$ $\mathrm{error}$",fontsize=16)
plot_title = r"$\mathrm{Buoyancy}$ $2^{nd}$ $\mathrm{derivative}$ $\mathrm{error}$"
plt.title(plot_title,fontsize=16)
plt.yticks([1e-8,1e-6,1e-4,1e-2])
plt.grid(); plt.legend(loc=3,fontsize=16)
